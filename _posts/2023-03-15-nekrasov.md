...
One can then use the $$ GL(V)$$ orbits, look at fixed points, and consider $$V$$ as a $$\tilde{T}-$$ module via some simple algebra. We can then modify the map of complexes above to make it $$T^{r+2}-$$equivariant. 