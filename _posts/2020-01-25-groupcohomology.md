...
# Group Cohomology and a few applications
...
Associativity of multiplication in  $$ E $$(I leave it to the reader to check this) gives the equality:
 ...
 $$ g.f(h,k)-f(gh,k)+f(g,hk)-f(g,h)=0 $$ which is exactly the condition for  $$ f $$ to be in  $$ Z(G,A) $$. Finally, let  $$ \sigma_{1} $$ be another section of the extension and let  $$ f_{1} $$ be its factor set. We'll show next that  $$ f $$ and  $$ f_{1} $$ differ by a coboundary, an element of  $$ B(G,A) $$ which implies that every extension has a well-defined cohomology class in  $$ H^{2}(G,A) $$.
...
(omitted here, or left as exercise)
